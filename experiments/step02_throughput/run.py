...
from experiments.step02_throughput.config import DEFAULT_DURATION, DEFAULT_MAIN_RATES, DEFAULT_SEED, DEFAULT_SIDE_RATES
...
def run(main_rates: list[int], side_rates: list[int], duration: float, seed: int) -> Path:
    ...
    csv_path = run_load_sweep(main_rates, side_rates, duration, seed=seed, csv_path=results_dir / "throughput.csv")
    write_metadata(results_dir / "metadata.json", {
        "experiment_id": "step02_throughput_sweep",
        "script": "experiments/step02_throughput/run.py",
        "csv": "experiments/step02_throughput/results/throughput.csv",
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run Step 2 throughput and capacity sweep.")
